...
MARGIN_PAGES_DISPLAYED = PAGINATION_SETTINGS.get("MARGIN_PAGES_DISPLAYED", 1)
...
def paginator(request, qs, per_page=12):
    page = request.GET.get('page', 1)
    _paginator = Paginator(request=request, object_list=qs, per_page=per_page)
    try:
        qs = _paginator.page(page)
    except PageNotAnInteger:
        qs = _paginator.page(1)
    except EmptyPage:
        qs = _paginator.page(_paginator.num_pages)
    return qs
