...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        if (typeof window.CKEDITOR !== 'undefined' && Object.keys(window.CKEDITOR.instances).length) {
    ...
    $(document).on('click', '.delete-autosave', function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (confirm(translate_static_words('آیا از حذف متن ذخیره شده خودکار مطمئنید؟', "Are you sure you want to delete your autosave?"))) {
            $(e.target).closest('li').fadeOut('fast');
    ...
    $(document).on('locking:disabled', function () {
        $('#autosave-message').hide();
    });
    $(document).on('locking:enabled', function () {
    ...
    DjangoAutosave.onCKEditorLoad = function (callback) {
        ...
        var $textareas = CKEDITOR.instances;
        var totalEditors = Object.keys($textareas).length;
        ...
        switch (CKEDITOR.status) {
            ...
            case 'ready':
                return callback();
            default:
                for (const tx of Object.entries($textareas)) {
                    var $textarea = $(tx);
                    if ($textarea && $textarea[1]['status'] == 'ready') {
                        totalEditors--;
                    }
                }
                if (totalEditors === 0) {
    ...
    DjangoAutosave.contentIsDifferent = function () {
        ...
        for (var i = savedData.formValues.length - 1; i >= 0; i--) {
            ...
            if (savedData.formValues[i].name === 'csrfmiddlewaretoken') {
                continue;
            }
    ...
    DjangoAutosave.suggestRevert = function (last_autosaved) {
        ...
        var fa_msg = [
            "به نظر میرسد شما در تاریخ ",
            Date(last_autosaved).toLocaleString(),
            " یک متن ذخیره نشده دارید",
            '. <a href="#revert-to-autosaved" class="revert-to-autosaved">بازیابی تغییرات</a> یا ',
            ' <a href="#ignore-autosaved" class="ignore-autosaved">ادامه با نسخه فعلی</a>؟'
        ].join('');
        var msg = translate_static_words(fa_msg, eng_msg)
