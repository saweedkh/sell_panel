...
class AdminAutoSaveMixin(object):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if 'is_retrieved_from_autosave' in request.POST:
            ...
            refresh_action = _('بازیابی فایل اصلی') if obj else _('پاک کردن فرم')
            msg = mark_safe((_(f'آخرین نسخه ذخیره شده با موفقیت بازیابی شد. '
                               f'برای <a href="">{refresh_action}</a> کلیک کنید. ')
                             ))
